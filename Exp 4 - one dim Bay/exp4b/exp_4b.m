...
%     filePath1 = fullfile('C:\Users\rutge\Documents\MATLAB\Thesis\Exp 1 - one dim Freq\exp_1b', [variableNames{i}, '.mat']);
...
plot(Plot(:,1),Plot(:,2),'ko');
hold on
plot(Plot(:,1),Plot(:,5),'b', 'LineWidth', 2);
plot(Plot(:,1),Plot(:,3),'r--', 'LineWidth', 2)
plot(Plot(:,1),Plot(:,4),'r--', 'LineWidth', 2);
...
%ylim([-2 2])
%xlim([-100 100])
legend('Test input','Prediction','Bayesian $95\%$ confidence interval','','Location','northwest','interpreter','latex','FontSize', 12)
