...
for r = 1:R
%W_mean{1}(:,r) = W_mean{1}(:,r) * norms(r) ;
%W_mean{2}(:,r) = W_mean{2}(:,r) * norms(r) ;
%W_mean{3}(:,r) = W_mean{3}(:,r) * norms(r) ;
end

% Z1 = features(X,1,M);
% Z2 = features(X,2,M);
% Z3 = features(X,3,M);
% ZW = (Z1*W_mean{1}).*(Z2*W_mean{2}); 
% 
% [CC,Cy,~] = C_matrices(Z3,ZW,y) ;
% P0_inv = inv(P0{m}) ;
% z = (CC./sigmae + P0_inv) ;
% x = (z \ (Cy./sigmae + P0_inv*m0{m})) ;
% W_cov{m} = inv(z) ;
...
for i = 1:M_fac
x_i{i} = reshape(x_i{i},2*R,[]) ;
x_i_M{i} = reshape(x_i_M{i},2*R,[]) ;

for d = 1:D
x_0_res{d} = reshape(x_0(:,d), [2 R]) ;

x_i_res{i}{d} = reshape(x_i{i}(:,d), [2 R]) ;

x_i_M_res{i}{d} = reshape(x_i_M{i}(:,d), [2 R]) ;
...
for m = 1:D-1
    if i == 1
W_x_0 = khatri_rao(W_x_0, x_0_res{m+1});
    end
W_x_i{i} = khatri_rao(W_x_i{i}, x_i_res{i}{m+1});
W_x_i_M{i} = khatri_rao(W_x_i_M{i}, x_i_M_res{i}{m+1});
end
