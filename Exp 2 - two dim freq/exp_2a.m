...
variableNames = {'X', 'XTest', 'Y', 'YTest','XPlot','lambda_delta'};
...
% Confidence bounds
% [var_CI, var_PI, CI, PI] = lin_uncertainty_multi(W, X, XPlot, Y, norms) ;

% Standard deviation
% lambda = sqrt(pred_error_var + approx_var) ;
% lambda = sqrt(pred_error_var) ;
...
%load("CI_lower_delta.mat","CI_upper_delta.mat","CI_lower_Core.mat","CI_upper_Core.mat","pred_delta.mat","pred_Core.mat")
...
%    fig = gcf;
    ...
    b1 = contour(X1Plot,X2Plot,reshape(pred_delta,size(X1Plot)),[0 0],'green','LineWidth',1.5);
    b2 = contour(X1Plot,X2Plot,reshape(CI_lower_delta,size(X1Plot)),[0 0],'green','LineWidth',1.5,'LineStyle','--');
    b3 = contour(X1Plot,X2Plot,reshape(CI_upper_delta,size(X1Plot)),[0 0],'green','LineWidth',1.5,'LineStyle','--');
...
[var_CI, var_PI] = delta_method(C_train, C_test, lambda, regu) ;
