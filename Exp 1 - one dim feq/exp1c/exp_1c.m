...
X = readmatrix('power_plant.csv'); 

x1 = linspace(-10, 10, 20);      % Range for dimension 1
...
XTest_ref = (XTest-XMin)./(XMax-XMin);
YTest = (YTest-YMean)./YStd;

[W, norms, reg] = CP_ALS_weights(X, Y, M, R,lambda, maxIte) ;
    
% Prediction
pred = CP_ALS_predict(XTest_ref,W,norms,M,X,reg) ;
pred_ref = real(pred) ;

norms = ones(R,1) ;
%pred_data_fit = CP_data_gen(X, W) ;
